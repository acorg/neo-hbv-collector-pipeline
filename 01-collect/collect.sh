...
function collect()
{
    ...
    sample=$(sampleName)
    sampleIndex=$(egrep " $sample\$" ../../../refseq-diamond/samples.index | cut -f1 -d' ')
    hbvIndex=$(egrep ' Hepatitis B virus$' ../../../refseq-diamond/pathogens.index | cut -f1 -d' ')
    ...
    then
        ...
        # ugly and this count will be very quick as we only have at most a
        # few thousand HBV reads for a sample.
